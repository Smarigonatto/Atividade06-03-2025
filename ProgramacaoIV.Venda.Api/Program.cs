...
app.MapGet("/vendedores", async (VendaContext context) =>
    await context.Vendedor.Where(x => x.IsAtivo).ToListAsync());

app.MapGet("/vendedores/{id}", async (string id, VendaContext context) =>
    await context.Vendedor.Where(x => x.Id == Guid.Parse(id) && x.IsAtivo).FirstOrDefaultAsync() is Vendedor vendedor
        ? Results.Ok(vendedor)
        : Results.NotFound());
...
{
    var vendedor = await context.Vendedor.Where(x => x.Id == Guid.Parse(id) && x.IsAtivo).FirstOrDefaultAsync();
...
app.MapGet("/transacoes", async (VendaContext context)
    => await context.Transacoes
            .Include(x => x.Cliente)
            .Include(x => x.Vendedor) // Inclui o vendedor na transação
            .Include(x => x.Itens)
...
app.MapGet("/transacoes/{id}", async (string id, VendaContext context)
    => await context.Transacoes
            ...
            .Include(x => x.Itens)
                .ThenInclude(x => x.Produto)
            .Where(x => x.Id == Guid.Parse(id) && x.IsAtivo).FirstOrDefaultAsync() is Transacao produto
            ? Results.Ok(produto)
            : Results.NotFound());
...
app.MapPost("/transacoes", async ([FromBody] TransacaoCapaRequest request, VendaContext context) =>
{
    ...
    var vendedor = await context.Vendedor.Where(x => x.Id == request.IdVendedor && x.IsAtivo).FirstOrDefaultAsync();

    if (vendedor is null)
        return Results.NotFound("Vendedor não encontrado.");

    var transacao = new Transacao(cliente, vendedor); // Cria a transação com o vendedor
